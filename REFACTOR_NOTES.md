...
- 本仓库由 Codex 与 Claude 两个执行体同时维护，协作规则见 `AGENTS.md`，以该文件为准。
- 本文件是项目内容与进度的真源；`AGENTS.md` 是协作规则的真源。
- 本文件的「原则」「① 工程事实表」「③ 偏差记录」「DSP 解冻记录」只能在 main 上单独提交修改。
- 特性分支只允许向「④ 进度表 / 已完成」追加自己卡片的执行记录。

> 完整目标和交接背景见 [PROJECT_HANDOFF.zh-CN.md](PROJECT_HANDOFF.zh-CN.md)。
> 下方 ①② 是 T01 勘察快照；行号、两 Tab 描述和“当前”字样不再代表最新代码，④ 按主干现状持续维护。
...
- 原则：`SSTVKit/Sources/SSTVKit` 是受保护的 DSP 核心。未经用户明确书面授权，
  不得改变 VIS 编解码、行时序与行时钟、色彩转换、重采样和 tone 频率映射的行为。
  每一次解冻必须登记在下方「DSP 解冻记录」表；没有登记的解冻视为违规。
  协作规则见 `AGENTS.md`。
...
- 每块使用 `AGENTS.md` 规定的执行体前缀分支，立即提供 Actions 链接；CI 通过后才合主干，失败在同分支修复，不带红灯进入下一步。
...
| # | 日期 | 授权摘要 | 涉及文件 | 卡片 | 执行体 |
|---|---|---|---|---|---|
| U01 | 2026-09-05 | 用户要求修复“图片要么弯曲，要么颜色不对”，并补充“如果晚接收几秒就不能识别信息了……爱好者将失去一次通联机会”。 | `SSTVFrameAssembler.swift`（行时钟改为 `SSTVLineClock`）、`SSTVDecoder.swift`、`SSTVFrequencyDemodulator.swift`、`SSTVToneSampler.swift`、`SSTVDecodeTypes.swift`，新增 `SSTVLineClock.swift`、`SSTVLateEntryDetector.swift`、`SSTVAudioBandFilter.swift` | 原 `ui/receive-quality` | codex |

## ④ 进度表

### 总进度

状态按当前 `main` 更新；“部分完成”不得被解释为对应步骤已经通过完整产品验收。

| 步骤 | 内容 | 负责人 | 状态 |
|---|---|---|---|
| 0 | 纯算法输入的三模式 WAV 基线与独立行序诊断 | codex | 已完成：三个原始 WAV 与哈希已核验归档；行序诊断独立记录，两个模式未定 |
| 1 | Theme、颜色资产、通用组件 |  | 已完成 |
| 2 | 接收默认的四 Tab 骨架 |  | 已完成；图库与设置仍是明确占位页 |
| 3 | 图库存储与图库页 | codex | 部分完成：存储、恢复、网格、详情、多选分享、收发入库及按需存入系统相册已通过 CI 并进入 main；48 kHz 收发性能与真机验收待完成 |
| 4 | 发射页、真实时长、进度、叠字与播放 |  | 未开始 |
| 5 | 接收状态条、电平、连续收图、15 fps 快照显示 |  | 部分完成：接收质量与中途锁定已交付；本步骤 UI、仪表和连续收图未完成 |
| 6 | 1024 点 Hann FFT 与 20 fps 瀑布图 |  | 未开始 |
| 7 | 设置页 |  | 未开始 |
| 8 | 原 UI 稿十项自检与 WAV 哈希回归 |  | 部分完成：冻结 WAV 哈希门禁已通过；十项整体验收未运行 |

旧卡片编号仅作勘察历史引用，不扩大本轮范围；第 0 步先报告三个真实哈希，再进入后续 UI。

### 已完成

| 卡片 | 执行体 | 完成时间 | CI run 链接 | 结果 |
|---|---|---|---|---|
