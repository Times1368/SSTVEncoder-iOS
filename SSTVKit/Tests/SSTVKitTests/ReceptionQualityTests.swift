...
final class ReceptionQualityTests: XCTestCase {
    ...
    private func checkPattern(mode: SSTVMode, sampleRate: Int) async throws {
        ...
        for row in 0..<radioLines {
            writer.append(contentsOf: try SSTVLineEncoder.segments(for: source, mode: mode, row: row))
        }
        writer.append(frequencyHz: 1_900, duration: 0.04)
        let decoder = try SSTVStreamDecoder(sampleRate: sampleRate)
        var offset = 0
        while offset < writer.samples.count {
            let end = min(offset + 2_048, writer.samples.count)
            _ = try await decoder.append(Array(writer.samples[offset..<end]))
            offset = end
        }
        let frame = try await decoder.finish()
        let expectedRows = mode.family == .pd ? radioLines * 2 : radioLines
        let label = "\(mode.displayName) / \(sampleRate) Hz"
        ...
        let report = Report(
            ...
            frequencyOffsetHz: frame.frequencyOffsetHz
        )
    ...
    private func writeArtifacts(report: Report, source: RGBImage, decoded: RGBImage) throws {
        ...
        let stem = "\(report.mode)-\(report.sampleRate)"
