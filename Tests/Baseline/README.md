...
`EncoderBaselineTests` 只生成待比较的验证副本，不覆盖本目录冻结文件。
...
目标架构、构建配置与输入，不更新冻结值来让测试通过。

## 2026-09-06 基线分支整合

`ui/baseline-integration` 从已整合绿色 App 代码的 main 分出，再合入 `ui/baseline`
的三个独有提交（包含原来的 `codex/t02-baseline-capture` 历史）。合并冲突仅涉及本说明，
并保留后续资产校验、版本 1.1.3（5）、接收测试与固定环境编码哈希门槛。
App 和 `SSTVKit/Sources/SSTVKit` 生产源码与 main 不变。

原始采集 job 仍只针对历史 `ui/baseline` 分支；本次整合不触发它，也不重跑旧 workflow。
整合 CI 会构建/测试合并后的 SwiftPM targets，并执行既有的编码哈希、App 测试、IPA 打包和复验。
这不是重新采集或重定义冻结基线。该整合提交须通过 CI 后才能进入 main。

本次 Windows 检查：36 项脚本测试通过、仓库契约通过；三个归档 WAV 的 SHA-256、
PCM 格式、样本数、数据长度和时长重新只读核对通过。生产源码与 main 相同，
原始 WAV、SHA256SUMS 和元数据与 `8018416` 相同。未在 Windows 执行 Swift/Xcode。
